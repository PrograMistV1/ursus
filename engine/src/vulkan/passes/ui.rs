...
impl UiPass {
    pub fn record(
        ...
        unsafe {
            transition_swapchain(device, cmd, swapchain_image,
        ...
        egui.end_frame_and_draw(
            window, graphics_queue, command_pool,
            cmd, extent, egui_output,
        )?;
...
fn transition_swapchain(
    ...
    let (src_stage, src_access, dst_stage, dst_access) = match (from, to) {
        ...
        _ => panic!("transition_swapchain: неизвестная пара {:?} → {:?}", from, to),
    };
